...
void print_arr(int const* arr, unsigned size) {
    for (unsigned i = 0; i < size; i++) printf("%d ", arr[i]);
    printf("\n");
}
...
void selection_iteration(int* arr, unsigned size, unsigned sorted_pos) {
    ...
    if (arr[min_pos] < arr[sorted_pos]) swap(&arr[sorted_pos], &arr[min_pos]);
}

void selection_sort(int* arr, unsigned size) {
    for (unsigned i = 0; i < size; i++) selection_iteration(arr, size, i);
}
